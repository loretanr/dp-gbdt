...
def create_plot(filename):
    ...
    data = pd.read_csv(filename, usecols=[
        'dataset', 'nb_samples', 'nb_trees', 'privacy_budget', 'mean', 'std', 'glc', 'gdf', 'balanced'])
    ...
    # plot the dp curve (GDF-balanced)
    values_dp = data[(data['nb_samples'] == SAMPLES) & (data['privacy_budget'] != 0) & (data['balanced'] == True) & (data['gdf'] == True)]
    mean = values_dp['mean']
    std = values_dp['std']
    marker = '-^'
    label = 'DP-GBDT (gdf/balanced)'
    ...
    # plot the dp curve (GDF-unbalanced)
    values_dp = data[(data['nb_samples'] == SAMPLES) & (data['privacy_budget'] != 0) & (data['balanced'] == False) & (data['gdf'] == True)]
    mean = values_dp['mean']
    std = values_dp['std']
    marker = '-^'
    label = 'DP-GBDT (gdf/unbalanced)'
    plt.errorbar(values_dp['privacy_budget'], mean, yerr=std,
                 fmt=marker, capsize=3, label=label, markersize=1, color='mediumvioletred', ecolor='dimgrey', elinewidth=1)
