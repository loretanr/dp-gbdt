...
def create_plot(filename, ymin, ymax):
    ...
    data = pd.read_csv(filename, usecols=[
        'dataset', 'nb_samples', 'nb_trees', 'privacy_budget', 'mean', 'std', 'glc', 'gdf', 'mean_mape', 'std_mape'])
    ...
    plt.annotate('baseline (mean)', xy=(0.1, 24.6), xycoords='data', color = 'mediumseagreen')

    # plot the dp curve (GDF)
    values_dp = data[(data['nb_samples'] == SAMPLES) & (data['privacy_budget'] != 0) & (data['glc'] == False) & (data['gdf'] == True)]
    mean = values_dp['mean_mape']
    std = values_dp['std_mape']
    marker = '-^'
    label = 'DP-GBDT (gdf)'
    ...
    if not values_no_dp.empty:
        mean = [values_no_dp.at[0,'mean_mape']] * values_dp.shape[0]
        std = [values_no_dp.at[0,'std_mape']] * values_dp.shape[0]
        marker = '-v'
        label = 'GBDT no-dp'
        plt.errorbar(values_dp['privacy_budget'], mean, yerr=std,
    ...
    plt.yticks([14,16,18] + np.arange(20, ymax+1, 5).tolist())
