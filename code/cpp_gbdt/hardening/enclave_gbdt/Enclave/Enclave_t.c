...
typedef struct ms_ecall_load_modelparams_into_enclave_t {
	struct sgx_modelparams* ms_mparams;
...
static sgx_status_t SGX_CDECL sgx_ecall_load_dataset_into_enclave(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	struct sgx_dataset* _tmp_dset = ms->ms_dset;
	size_t _len_dset = sizeof(struct sgx_dataset);
	struct sgx_dataset* _in_dset = NULL;

	CHECK_UNIQUE_POINTER(_tmp_dset, _len_dset);
	...
	if (_tmp_dset != NULL && _len_dset != 0) {
		_in_dset = (struct sgx_dataset*)malloc(_len_dset);
		if (_in_dset == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_dset, _len_dset, _tmp_dset, _len_dset)) {
	...
	ecall_load_dataset_into_enclave(_in_dset);

err:
	if (_in_dset) free(_in_dset);
	return status;
}
...
static sgx_status_t SGX_CDECL sgx_ecall_load_modelparams_into_enclave(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	struct sgx_modelparams* _tmp_mparams = ms->ms_mparams;
	size_t _len_mparams = sizeof(struct sgx_modelparams);
	struct sgx_modelparams* _in_mparams = NULL;

	CHECK_UNIQUE_POINTER(_tmp_mparams, _len_mparams);
	...
	if (_tmp_mparams != NULL && _len_mparams != 0) {
		_in_mparams = (struct sgx_modelparams*)malloc(_len_mparams);
		if (_in_mparams == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_mparams, _len_mparams, _tmp_mparams, _len_mparams)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ecall_load_modelparams_into_enclave(_in_mparams);

err:
	if (_in_mparams) free(_in_mparams);
	return status;
}
